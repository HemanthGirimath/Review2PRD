<template>
  <div class="app">
    <!-- Sticky CTA Banner -->
    <div class="demo-sticky-banner">
      <span class="banner-text"><strong>Want this for your own app?</strong> Join the waitlist to turn your reviews into actionable tickets in under 2 minutes.</span>
      <router-link to="/" class="banner-cta">Get early access</router-link>
    </div>
    ...
    <main class="app-main">
      <!-- Input -->
      <div v-if="step === 'input'">
        <div class="card main-card" style="padding: 3rem; text-align: center;">
          <h2 style="font-size: 1.5rem; margin-bottom: 1rem; color: #fff;">Try an Interactive Demo</h2>
          <p style="margin-bottom: 2rem; color: var(--color-text-muted);">Select an app below to see exactly how Review2PRD turns reviews into actionable insights.</p>
          <div style="display: flex; gap: 1rem; justify-content: center;">
            <button @click="startDemo('zomato')" class="demo-select-btn" style="padding: 1rem 2rem; border-radius: 8px; border: 1px solid var(--color-border); background: var(--color-surface-2); color: #fff; cursor: pointer; font-size: 1.1rem; transition: background 0.2s;">🍔 Zomato</button>
            <button @click="startDemo('bumble')" class="demo-select-btn" style="padding: 1rem 2rem; border-radius: 8px; border: 1px solid var(--color-border); background: var(--color-surface-2); color: #fff; cursor: pointer; font-size: 1.1rem; transition: background 0.2s;">💛 Bumble</button>
          </div>
        </div>
      </div>
...
import { fakeSpotifyRes as fakeZomatoRes, fakeBumbleRes } from '../lib/fakeData'
...
async function startDemo(appChoice: 'zomato' | 'bumble') {
  step.value = 'scraping'
  loadingMessage.value = `Scraping reviews for ${appChoice === 'zomato' ? 'Zomato' : 'Bumble'}…`
  
  await new Promise(r => setTimeout(r, 1500))
  
  step.value = 'analyzing'
  loadingMessage.value = `Analyzing complaints with AI…`
  ...
  const data = appChoice === 'zomato' ? fakeZomatoRes : fakeBumbleRes
  prd.value = data.prd as any
  issues.value = data.issues as any
  ticketCache.value = data.ticket_cache as any
  
  step.value = 'result'
  activeView.value = 'prd'
}
</script>
