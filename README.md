...
The agent uses a local SQLite database at `db/prospects.db` to track the history of sent emails and deduplicate prospects.

### 1. Check Sent Logs / Drafted Emails
To see all drafts and check which emails succeeded, failed, or were skipped:
```bash
sqlite3 db/prospects.db "SELECT email, status, subject, body, error_message FROM email_logs;"
```

### 2. Clear Sent Logs (Reset Run History)
To clear only your history logs so you can run the batch again for testing:
```bash
sqlite3 db/prospects.db "DELETE FROM email_logs;"
```

### 3. Clear Master Prospects List
To clear the master list of prospects entirely:
```bash
sqlite3 db/prospects.db "DELETE FROM prospects;"
```

### 4. Delete the Database File (Full Reset)
To delete both tables and start entirely from scratch:
```bash
rm db/prospects.db
```
